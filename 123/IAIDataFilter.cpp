...
#include <utility>
	...
	#include "AutoSuite.h"
	use_suite_required(AIDataFilter)
...
namespace ai
{
	...
	class ZStdDataFilter : public BaseDataFilter
	{
	public:
		using inherited = BaseDataFilter;
		explicit ZStdDataFilter(IDataFilter::Mode filterMode)
			:inherited(filterMode)
		{
			const auto aiMode = GetAIMode();
			mError = sAIDataFilter->NewZStdDataFilter(aiMode, &mFilter);
		}
	};

	class AsyncDataFilter : public BaseDataFilter
	{
	public:
		using inherited = BaseDataFilter;
		explicit AsyncDataFilter(IDataFilter::Mode filterMode,
		...
		{
			const auto aiMode = GetAIMode();
			mError = sAIDataFilter->NewAsyncDataFilter(aiMode, asyncFilter->ReleaseFilter(), prefetchSize, outputSize, &mFilter);
		}
		AsyncDataFilter(AsyncDataFilter&& infilter) = delete;
	};
	...
	std::unique_ptr<ai::IDataFilter> CreateZStdDataFilter(
		IDataFilter::Mode filterMode,
		std::unique_ptr<IDataFilter> nextFilter)
	{
		std::unique_ptr<ai::IDataFilter> filter(std::make_unique<ZStdDataFilter>(filterMode));
		if (filter && nextFilter)
		{
			filter->Link(std::move(nextFilter));
		}
	...
	{
		std::unique_ptr<ai::IDataFilter> filter(std::make_unique<AsyncDataFilter>(filterMode, std::move(asyncFilter), prefetchSize, outputSize));
