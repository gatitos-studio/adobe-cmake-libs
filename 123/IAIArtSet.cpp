...
#include <stdexcept>
	...
	#include "AutoSuite.h"
	use_suite_required(AIArtSet)
	use_suite_required(AIArtSetGenerator)
...
namespace ai
{
	...
	AIErr ArtSet::Impl::GenerateNSelectedAtTopLevel(size_t inMaxSelectedArtsToReturn,
													bool *outHasMoreSelectedArts)
	{
		AIBoolean hasMoreSelectedArts{ false };
		mLastError = sAIArtSetGenerator->GenerateNSelectedAtTopLevel(inMaxSelectedArtsToReturn, mArtSet, (outHasMoreSelectedArts ? &hasMoreSelectedArts : nullptr));
		if (mLastError == kNoErr && outHasMoreSelectedArts)
		{
			*outHasMoreSelectedArts = !!hasMoreSelectedArts;
		}
		return mLastError;
	}
	...
	AIErr ArtSet::GenerateNSelectedAtTopLevel(size_t inMaxSelectedArtsToReturn, bool *outHasMoreSelectedArts)
	{
		return pImpl->GenerateNSelectedAtTopLevel(inMaxSelectedArtsToReturn, outHasMoreSelectedArts);
	}
